# for final 5, changed slope function
...
def sortScore(stockName): #2, 3 , 9 , 10
    ...
    # 10) sector
    # 12) slope
    # print(stockName + " " + str(score1(stockName))+ " "+str(score5(stockName))+ " "+str(score6(stockName))+ " "+str(score11[stockName])+ " "+str(score12(stockName)) + " "+str(getDoubleDerivative(stockName)))
    score = score1(stockName)*score6(stockName)*score5(stockName)
    # print(stockName + " "+str(getBrickSize(stockName)))
    if getDoubleDerivative(stockName) == 2:
        score *= 5
...
def getSlope(stockName):
    ...
    slope = numGreen * getBrickSize(stockName)
    numDays = (dates[-1] - dates[-1*numGreen])
    if numGreen == 1:
        return 0
    else:
        slope /= abs(((numDays.total_seconds())))
    return slope
...
def getDoubleDerivative(stockName):
    ...
    numDays2 = dates[-1*numGreen] - dates[mid]
